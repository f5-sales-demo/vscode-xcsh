...
export class CertAuthProvider implements AuthProvider {
  ...
  private createAgentFromP12(): https.Agent {
    ...
    try {
      ...
      // Extract private key
      const bagTypeIdentifier = forge.pki.oids.pkcs8ShroudedKeyBag as string;
      const keyBags = p12.getBags({ bagType: bagTypeIdentifier });
      let keyBagArray = keyBags[bagTypeIdentifier];
