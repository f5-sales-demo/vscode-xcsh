...
/**
 * Serialize per-resource read-only view layouts into a self-contained TS module.
 * Consumed by the describe/view panel to render every resource with labelled,
 * ordered, sectioned fields (spec-driven baseline; curated section manifests layer
 * exact console titles on top).
 */
export function generateViewLayoutsContent(specs: ParsedSpecInfo[]): string {
  const sorted = [...specs].sort((a, b) => a.resourceKey.localeCompare(b.resourceKey));

  const layouts: Record<string, ResourceViewLayout> = {};
  for (const spec of sorted) {
    if (spec.viewLayout && spec.viewLayout.fields.length > 0) {
      layouts[spec.resourceKey] = spec.viewLayout;
    }
  }

  const layoutsJson = JSON.stringify(layouts, null, 2).replace(/"([a-zA-Z_$][a-zA-Z0-9_$]*)":/g, '$1:');

  return `/**
 * Auto-generated resource view layouts from F5 XC OpenAPI specifications.
 * DO NOT EDIT - This file is generated by scripts/generate-resource-types.ts
 *
 * Each layout is the read-only field tree (labels/order/read-only) derived from a
 * resource's GetSpecType schema. The describe panel renders these; curated section
 * manifests (src/providers/viewSectionManifests.ts) group them into console titles.
 *
 * Total resource layouts: ${Object.keys(layouts).length}
 */
...
export const GENERATED_VIEW_LAYOUTS: Record<string, ResourceViewLayout> = ${layoutsJson};
`;
}

/**
 * Generate and write the view layouts module.
 */
export function writeViewLayoutsFile(specs: ParsedSpecInfo[], outputPath: string): void {
  const content = generateViewLayoutsContent(specs);
  const outputDir = path.dirname(outputPath);
  if (!fs.existsSync(outputDir)) {
    fs.mkdirSync(outputDir, { recursive: true });
  }
  fs.writeFileSync(outputPath, content, 'utf-8');
  console.log(`Generated: ${outputPath} with ${specs.filter((s) => s.viewLayout).length} view layouts`);
}
